...
var quattroAdminLevels = [
  ...
  },
  {
    name: 'admin2',
    path: 'adm2',
    props: [ 'qs_a2', 'qs_a2_alt', 'popularity' ]
  },
  {
    name: 'local_admin',
    path: 'localadmin',
...
function createLookup( workers ){
  ...
  function completeSearch( id ){
    ...
    if( result.alpha3 !== undefined && result.admin1 !== undefined ){
      var alpha3 = result.alpha3.toLowerCase();
      if( alpha3 in stateAbbreviations ){
        result.admin1_abbr = stateAbbreviations[ alpha3 ][ result.admin1 ];
      }
    }
...
function createLookupStreamSync(){
  ...
  var loggerInterval = setInterval( function (  ){
  ...
  function write( model, _, next ){
    function execSearch(){
      ...
      lookup.search( model.getCentroid(), function searchCb( results ){
        ...
        if( model.getAdmin( 'admin1_abbr' ) === undefined ){
          try {
        ...
        if( model.getPopulation() === undefined ){
          try {
            model.setPopulation( results.population );
          }
          catch ( ex ){
            stats.failures.population++;
          }
        }

        if( model.getPopularity() === undefined ){
          try {
            model.setPopularity( results.popularity );
          }
          catch ( ex ){
            stats.failures.popularity++;
          }
        }

        adminLevelNames.forEach( function ( name ){
          if( model.getAdmin( name ) === undefined ){
            try {
